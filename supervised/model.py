...
class SpatialHeteroConv(nn.Module):
    ...
    def _attn_aggregate(self, q, k, v, edge_src, edge_dst,
        ...
        log_sigma = self.dist_kernel.log_sigma
...
def recon_loss(h_spot, h_gene, h_peak, data, n_samples=30000):
    ...
    if sg_total > 0:
        ...
        pred = (h_gene[data['sg_gene_idx'][idx]] * h_spot[data['sg_spot_idx'][idx]]).sum(dim=-1) / scale
    ...
    if sp_total > 0:
        ...
        pred = (h_peak[data['sp_peak_idx'][idx]] * h_spot[data['sp_spot_idx'][idx]]).sum(dim=-1) / scale
...
class SupervisedTrainer:
    ...
    def evaluate(self, h_spot):
        """Clustering metrics vs Joint_clusters. P22 has no labels.
        ...
        if self.cluster_mode in ('joint_all', 'joint_e13p21'):
            # Per-time joint-label sets are disjoint, so the natural joint k
            # is the sum of unique label counts in E13 + P21.
            n_clusters = 0
            for t in (0, 1):
    ...
    def compute_gp_scores(self, chunk_size=1024):
        self.model.eval()
        with torch.no_grad():
            h0 = self.model.get_initial_embeddings(self.data)
            h = self.model(h0, self.data['edge_index_ss'], self.data['edge_index_sg'],
