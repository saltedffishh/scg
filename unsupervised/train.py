"""
Unsupervised Training Entry Point
=================================
No external labels. Self-supervised only:
  L = L_recon + λ1·L_align + λ2·L_spatial + λ3·L_gp

Supports GPU (CUDA/MPS) with mixed precision.
Evaluation: NMI/ARI vs Joint_clusters (external ground truth).

Usage:
  python unsupervised/train.py                          # default params
  python unsupervised/train.py --epochs 300 --svd 200   # better quality
  python unsupervised/train.py --device cuda --amp      # GPU with mixed precision
"""
...
from unsupervised.model import UnsupervisedEncoder, UnsupervisedTrainer


def main():
    parser = argparse.ArgumentParser(
        description='Unsupervised Spatial Hetero Graph Encoder')
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'output_unsup')
    ...
    # Config summary
    print("=" * 60)
    print("Unsupervised Spatial Heterogeneous Graph Encoder")
    print("=" * 60)
    ...
    print(f"  joint_cluster={args.joint_cluster}")
