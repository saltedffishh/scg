...
class SpatialHeteroConv(nn.Module):
    ...
    def _attn_aggregate(self, q, k, v, edge_src, edge_dst,
        ...
        Args:
            q: (N_target, d) query embeddings
            k: (N_source, d) key embeddings
            v: (N_source, d) value embeddings
            edge_src: (E,) source node indices (local to k/v)
            edge_dst: (E,) target node indices (local to q)
            dist: (E,) optional distances for Spot-Spot edges
            num_nodes: total output nodes
        Returns:
            agg: (num_nodes, d) aggregated messages
        """
        if num_nodes is None:
            num_nodes = q.size(0)

        # Reshape to multi-head
        q_mh = q.view(-1, self.num_heads, self.d_k)           # (N_tgt, H, dk)
        k_mh = k.view(-1, self.num_heads, self.d_k)           # (N_src, H, dk)
        v_mh = v.view(-1, self.num_heads, self.d_k)

        # Gather for edges
        k_e = k_mh[edge_src]   # (E, H, dk)
        v_e = v_mh[edge_src]   # (E, H, dk)
        q_e = q_mh[edge_dst]   # (E, H, dk)

        # Attention scores
        attn = (q_e * k_e).sum(dim=-1) / (self.d_k ** 0.5)   # (E, H)

        # Distance modulation for Spot-Spot
        if dist is not None:
            w = self.dist_kernel(dist).unsqueeze(-1)          # (E, 1)
            attn = attn * w
...
def recon_loss(h_spot, h_gene, h_peak, data, n_samples=30000):
    ...
    if sg_total > 0:
        ...
        pred = (h_gene[g_local] * h_spot[s_local]).sum(dim=-1)
    ...
    if sp_total > 0:
        ...
        pred = (h_peak[p_local] * h_spot[s_local]).sum(dim=-1)
...
class UnsupervisedTrainer:
    ...
    def evaluate(self, h_spot):
        """NMI/ARI vs Joint_clusters (external validation, not used in training)."""
    ...
    def compute_gp_scores(self, top_k=100):
        """Compute Gene-Peak association scores."""
        self.model.eval()
        with torch.no_grad():
            h0 = self.model.get_initial_embeddings(self.data)
            h = self.model(h0, self.data['edge_index_ss'], self.data['edge_index_sg'],
        ...
        h_gene = h[ns:ns + ng]
        h_peak = h[ns + ng:]
        scores = torch.sigmoid(h_gene @ h_peak.T)  # (n_genes, n_peaks)
