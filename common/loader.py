...
def load_combined_data(data_dir, device=None, use_svd=True, svd_dim=100):
    ...
    sg_true_vals = np.array(
        ...
        dtype=np.float32)
    sg_gene_idx_t = torch.tensor(sg_gene_local, dtype=torch.long)
    sg_spot_idx_t = torch.tensor(sg_spot_local, dtype=torch.long)
    sg_true_t = torch.tensor(sg_true_vals, dtype=torch.float32)
    ...
    sp_true_vals = np.array(
        ...
        dtype=np.float32)
    sp_peak_idx_t = torch.tensor(sp_peak_local, dtype=torch.long)
    sp_spot_idx_t = torch.tensor(sp_spot_local, dtype=torch.long)
    sp_true_t = torch.tensor(sp_true_vals, dtype=torch.float32)
